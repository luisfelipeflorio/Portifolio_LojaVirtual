...
class CartFlowTest(TestCase):
    ...
    def test_add_product_to_cart(self):
        response = self.client.post('/cart/add/', {'product_id': self.product1.id, 'quantity': 1})
    ...
    def test_add_multiple_quantity(self):
        self.client.post('/cart/add/', {'product_id': self.product1.id, 'quantity': 3})
        session = self.client.session
        self.assertEqual(session['cart'][str(self.product1.id)]['quantity'], 3)

    def test_update_cart_item_quantity(self):
        self.client.post('/cart/add/', {'product_id': self.product1.id, 'quantity': 2})
        self.client.post('/cart/update/', {'product_id': self.product1.id, 'quantity': 5})
        session = self.client.session
        self.assertEqual(session['cart'][str(self.product1.id)]['quantity'], 5)

    def test_remove_cart_item(self):
        self.client.post('/cart/add/', {'product_id': self.product1.id, 'quantity': 1})
        self.client.post('/cart/add/', {'product_id': self.product2.id, 'quantity': 1})
        session = self.client.session
        self.assertEqual(len(session['cart']), 2)
        self.client.post('/cart/remove/', {'product_id': self.product1.id})
        session = self.client.session
    ...
    def test_cart_total(self):
        ...
        self.assertContains(response, str(int(expected_total)))
